...
def histogram_of_gradients(img, pix):
    ...
    for (x, y) in pix:
        ...
        #principle_ori=0
        #principle_ori=np.argmax(all_hist)*bin_w
...
if __name__ == '__main__':
    ...
    #cv2.imwrite("outputs/panorama_z1.jpg", pano)
    '''for i in range(2,3):
        img00=cv2.imread(f'{IMGDIR}/000.jpg')
        img01=cv2.imread(f'{IMGDIR}/00{i}.jpg')
        pixels1, pixels2 = feature_matching(img00, img01)
        homo_matrix = align_pair(pixels1, pixels2)
        principle_img = stitch_blend(
            img00, img01, homo_matrix)
        principle_img=np.uint8(principle_img)
        cv2.imwrite(f"outputs/0000{i}.jpg", principle_img)'''